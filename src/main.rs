...
fn resolve(args: Vec<String>) -> Result<Vec<AgentConfig>, String> {
    ...
    if let Some(dir) = path_override {
        ...
        return Ok(with_claude(
            vec![AgentConfig::from_dir(dir, model_override, endpoint_override)?],
            claude_code,
        ));
    }
    if here {
        ...
        return Ok(with_claude(
            vec![AgentConfig::from_dir(cwd, model_override, endpoint_override)?],
            claude_code,
        ));
    }
...
impl AgentSession {
    ...
    fn spawn_claude_code(cfg: &AgentConfig, color: egui::Color32) -> Result<Self, String> {
        ...
        let mut child = Command::new(&claude)
            ...
            .stderr(Stdio::inherit())
            .spawn()
            .map_err(|e| format!("failed to spawn claude ({claude}) for {}: {e}", cfg.agent_id))?;
    ...
    match v["type"].as_str() {
        ...
        Some("result") => {
            ...
            if v["is_error"].as_bool().unwrap_or(false) {
                ...
                out.push(ChatEvent::Message { text: text.to_string() });
            }
...
mod claude_code_tests {
    ...
    fn init_becomes_ready_with_claude_code_backend() {
        ...
        assert_eq!(out.len(), 1);
        match &out[0] {
            ChatEvent::Ready { agent, model, backend, tools } => {
    ...
    fn result_emits_message_then_turn_end_with_usage() {
        ...
        let out = translate_claude_event(&v, "a");
        assert_eq!(out.len(), 2);
        assert_eq!(out[0], ChatEvent::Message { text: "done".into() });
        assert_eq!(out[1], ChatEvent::TurnEnd { prompt_tokens: 12, completion_tokens: 3 });
    }
    ...
    fn assistant_tool_use_becomes_tool_call() {
        ...
        assert_eq!(out.len(), 1);
        match &out[0] {
            ChatEvent::ToolCall { id, name, .. } => { assert_eq!(id, "tu1"); assert_eq!(name, "Read"); }
